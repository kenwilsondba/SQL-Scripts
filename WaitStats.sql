...
--PLE
select * from sys.dm_os_performance_counters where (counter_name like 'Page life expectancy%' and object_name like '%Buffer Manager%')  or counter_name = 'Batch Requests/sec' or counter_name = 'SQL Compilations/sec' or counter_name = 'SQL Re-Compilations/sec'


--Compilations
DECLARE @batch bigint,@comp bigint,@recomp BIGINT;

SELECT @batch = cntr_value
FROM sys.dm_os_performance_counters
WHERE counter_name = 'Batch Requests/sec';
SELECT @comp = cntr_value
FROM sys.dm_os_performance_counters
WHERE counter_name = 'SQL Compilations/sec';
SELECT @recomp = cntr_value
FROM sys.dm_os_performance_counters
...
FROM sys.dm_os_performance_counters
WHERE counter_name = 'Batch Requests/sec';
SELECT (cntr_value - @comp) / 10 AS 'SQL Compilations/sec'
FROM sys.dm_os_performance_counters
WHERE counter_name = 'SQL Compilations/sec';
SELECT (cntr_value - @recomp) / 10 AS 'SQL Re-Compilations/sec'
FROM sys.dm_os_performance_counters
WHERE counter_name = 'SQL Re-Compilations/sec';
...
--slow SPs
select db_name(database_id),object_name(object_id,database_id),* from sys.dm_exec_procedure_stats
